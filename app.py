...
def create_advanced_plot(df, threshold):
    ...
    fig.add_hrect(y0=0, y1=1.5, fillcolor="red", opacity=0.1, layer="below", line_width=0,
                  annotation_text="Зона риска заражения", annotation_position="top left", row=1, col=1)
    
    # График расхода
    fig.add_trace(
        go.Scatter(
            x=df['Hour'], y=df['Flow Rate (L/s)'], name='Расход (реальный)',
    ...
    fig.add_trace(
        go.Scatter(
            x=df['Hour'], y=expected_flow, name='Расход (ожидаемый)',
    ...
    fig.add_vrect(x0=2, x1=5, fillcolor="blue", opacity=0.1, layer="below", line_width=0,
                  annotation_text="MNF зона", annotation_position="top left", row=2, col=1)
    
    # График качества
    fig.add_trace(
        go.Scatter(
            x=df['Hour'], y=df['Water Age (h)'], name='Возраст воды',
            line=dict(color='#9b59b6', width=2.5), fill='tonexty',
    ...
    fig.update_layout(
        ...
        plot_bgcolor='white', font=dict(size=11), margin=dict(l=60, r=60, t=80, b=60)
    )
    
    return fig

def draw_network_simple(wn, pos, node_colors, isolated_pipes, ax):
    """Упрощенная отрисовка сети без NetworkX проблем"""
    # Рисуем трубы
    graph = wn.get_graph()
    for edge in graph.edges():
        ...
        is_isolated = any((edge[0] in pipe or edge[1] in pipe) for pipe in isolated_pipes)
    ...
    for i, node in enumerate(node_list):
        x, y = pos[node]
        circle = plt.Circle((x, y), 15, color=node_colors[i], 
...
def create_heatmap_network(wn, df, degradation):
    ...
    for edge in graph.edges():
        ...
        end_pos = pos[edge[1]]
        ax.plot([start_pos[0], end_pos[0]], [start_pos[1], end_pos[1]], 
               color='#95a5a6', linewidth=3, alpha=0.5, zorder=1)
...
with st.sidebar.expander("🔧 Стресс-тест насоса", expanded=True):
    pump_pressure = st.slider("Напор насоса (м)", 30, 60, 40, step=5, 
                              help="Проверка устойчивости системы")
...
if st.sidebar.button("🚀 ЗАПУСТИТЬ СИМУЛЯЦИЮ", use_container_width=True, type="primary"):
    with st.spinner("⏳ Расчет цифрового двойника..."):
        try:
            data, net = run_epanet_simulation(m_types[material], iznos, freq, pump_pressure, enable_valves)
...
if st.session_state['data'] is not None:
    ...
    with col2:
        ...
        st.metric(label="Давление min", value=f"{min_pressure:.2f} bar",
    ...
    with col3:
        ...
        st.metric(label="Потери воды", value=f"{lost_l:,.0f} L",
                 delta="⚠️" if lost_l > 5000 else None)
    
    with col4:
        damage = lost_l * price
        st.metric(label="Ущерб", value=f"{damage:,.0f} ₸",
                 delta=f"-{damage:.0f}" if damage > 0 else None, delta_color="inverse")
    
    with col5:
        if mnf_detected:
            st.metric(label="MNF аномалия", value=f"+{mnf_anomaly:.1f}%",
                     delta="Скрытая утечка", delta_color="inverse")
        else:
            st.metric(label="MNF статус", value="Норма",
                     delta=f"{mnf_anomaly:.1f}%", delta_color="normal")
    
    # Предупреждения
    if contamination_risk:
        st.error("⚠️ **ОПАСНОСТЬ ИНФИЛЬТРАЦИИ!** Давление упало ниже 1.5 bar. Риск загрязнения!")
    
    if mnf_detected:
        st.warning(f"🔍 **MNF АНОМАЛИЯ:** Ночной расход превышает норму на {mnf_anomaly:.1f}%.")
    
    st.markdown("---")
    
    # --- TABS ---
    tab1, tab2, tab3, tab4, tab5 = st.tabs([
        "📈 Гидравлика", "🗺️ Топология", "🔥 Риск-карта", "🔄 IoT данные", "📋 Отчеты"
    ])
    ...
    with tab2:
        ...
        with col_map:
            ...
            # Отрисовка
            draw_network_simple(wn, pos, n_colors, st.session_state['isolated_pipes'], ax)
        ...
        with col_control:
            ...
            if active_leak:
                ...
                if st.session_state['isolated_pipes']:
                    ...
                    affected = len(find_isolation_valves(wn, st.session_state['leak_node'])[1]) * 250
    ...
    with tab3:
        ...
        with col_r2:
            ...
            if sorted_probs and sorted_probs[0][1] > 40:
                st.error("⚠️ Срочная замена труб!")
    ...
    with tab4:
        ...
        if st.session_state['csv_data'] is not None:
            ...
            if 'Pressure (bar)' in csv_df.columns and 'Flow Rate (L/s)' in csv_df.columns:
                ...
                st.error("❌ CSV должен содержать: Hour, Pressure (bar), Flow Rate (L/s)")
        else:
            st.info("📁 Загрузите CSV файл в боковой панели для сравнения")
