...
def render_sidebar():
    ...
    with st.sidebar.expander("🏙️ City Selection", expanded=True):
        ...
        if auto_weather:
            ...
            season_temp = st.slider(
                "Temperature (°C)",
                min_value=-30, max_value=45, value=10, step=1,
    ...
    with st.sidebar.expander("⚙️ Network Parameters", expanded=True):
        ...
        pipe_age = st.slider(
            "Pipe Age (years)", min_value=0, max_value=60, value=15, step=1,
    ...
    with st.sidebar.expander("💰 Economic Parameters", expanded=True):
        water_tariff = st.number_input(
            "Water Tariff (₸/L)", min_value=0.1, max_value=2.0, value=0.55,
            step=0.05, format="%.2f"
        )

        leak_threshold = st.slider(
            "Leak Detection Threshold (bar)", min_value=1.0, max_value=5.0, value=2.7,
            step=0.1, help="Pressure below which leak is detected"
        )

        repair_cost = st.number_input(
            "Repair Deployment Cost (₸)", min_value=10_000, max_value=200_000,
    ...
    # ── Memory safety: purge previous results before new run ─────────────
    # This block executes on every sidebar render. The actual clear happens
    # only when the button is clicked (checked inside main()).
...
def main():
    ...
    # ── Sidebar (updates dark_mode in session state) ──────────────────────
    ...
    if config["run_simulation"]:
        ...
        # ── MEMORY SAFETY: release previous simulation data ───────────────
        # Drop the heavy wntr network + DataFrames from the previous run
        # before building new objects, then let Python GC reclaim the memory.
    ...
    # ═════════════════════════════════════════════════════════════════════
    # MAIN DASHBOARD (results available)
    # ═════════════════════════════════════════════════════════════════════
    ...
    with tab_map:
        ...
        with col_control:
            ...
            top_residuals = sorted(
                results["residuals"].items(), key=lambda x: -x[1]
            )[:8]
    ...
    with tab_econ:
        ...
        with roi_cols[0]:
            st.metric("📦 Sensor CAPEX", f"{econ['capex_kzt']:,.0f} ₸",
        ...
        with roi_cols[2]:
            ...
            st.metric("⏱ Payback Period", payback_label, payback_status,
        ...
        st.download_button(
            ...
            data=csv_data,
            file_name=f"shygyn_{results['city_config']['name']}_"
    ...
    with tab_stress:
        ...
        if results["n1_result"]:
            if "error" in results["n1_result"]:
                ...
                with n1_cols[0]:
                    st.metric("🏘️ Affected Residents", f"{n1['virtual_citizens']:,}",
                              "Virtual population")

                with n1_cols[1]:
                    st.metric("📍 Affected Nodes", len(n1["affected_nodes"]),
                              "Disconnected junctions")

                with n1_cols[2]:
                    st.metric("⏱ Time to Critical", f"{n1['time_to_criticality_h']:.1f} h",
                              "Tank depletion")
        ...
        # City-specific maintenance advisories
        if results["city_config"]["name"] == "Астана":
